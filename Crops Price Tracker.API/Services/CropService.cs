...
namespace Crops_Price_Tracker.Services
{
    ...
    DateTime endDate)
     ...
     DateTime endDate)
        ...
        public async Task<List<string>> GetCropNames(DateTime startDate, DateTime endDate)
        {
            startDate = startDate.Date;
            endDate = endDate.Date.AddDays(1).AddTicks(-1);
            ...
            return await _crop
        .Distinct<string>("commodity", filter)
        .ToListAsync();
        }


        public async Task<List<string>> GetMarketNames(DateTime startDate, DateTime endDate)
        {
            startDate = startDate.Date;
            endDate = endDate.Date.AddDays(1).AddTicks(-1);
